            ...
            try
            {
                ...
                foreach (string customFile in customFiles)
                {
                    ...
                    if (generateDiffs && Path.GetExtension(customFile).Equals(".p3d", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (File.Exists(baseGameFile))
                        {
                            ...
                            P3D.FileDiffNew fileDiff = new(baseGameFile, customFile);
                            ...
                            if (!fileDiff.AllNew)
                            {
                                ...
                                fileDiff.Diff.Write(outputFile);
                                ...
                                customFilesIniSB.AppendLine($"{iniPath}=Resources/P3D_Diffs/HandleDiff.lua");
                ...
                if (generateDiffs)
                {
                    ...
                    if (diffCount > 0)
                    {
                        try
                        {
                            System.IO.File.WriteAllText(Path.Combine(outputFolder, "Resources", "P3D_Diffs", "HandleDiff.lua"), Properties.Resources.HandleDiffLua);

                            // TODO: Make ini handling less awful

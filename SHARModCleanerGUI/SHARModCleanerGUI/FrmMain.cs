...
namespace SHARModCleanerGUI
{
    public partial class FrmMain : Form
    {
        ...
        private void Process(string inputFolder, string outputFolder, bool removeUnchanged, bool convertRSD, bool generateDiffs, string sharFolder, bool logCopied)
        {
                ...
                foreach (string customFile in customFiles)
                {
                    ...
                    if (generateDiffs && Path.GetExtension(customFile).Equals(".p3d", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (File.Exists(baseGameFile))
                        {
                            ...
                            {
                                outputFile = Path.Combine(outputFolder, "Resources", "P3D_Diffs", relativePath);
                                outputDir = Path.GetDirectoryName(outputFile);
                                if (string.IsNullOrWhiteSpace(outputDir))
                                {
                                    AddLog($"Could not get output directory for file \"{outputFile}\". Exiting.");
                                    return;
                                }
                                Directory.CreateDirectory(outputDir);
                                fileDiff.WriteDiff(outputDir, Path.GetFileName(outputFile));
            ...
            if (convertRSD || generateDiffs)
            {
                AddLog("AT THIS TIME, THIS TOOL DOES NOT HANDLE MODIFYING SETTINGS FILES.");

                if (convertRSD)
                {
                    AddLog("YOU WILL STILL NEED TO ADD `RequiredHack=OggVorbisSupport` TO `Meta.ini`.");
                }

                if (generateDiffs)
                {
                    AddLog("YOU WILL NEED TO DOWNLOAD `LuaP3DLib` MANUALLY.");
                    AddLog("YOU WILL NEED TO UPDATE `CustomFiles.ini` MANUALLY.");
                }
            }
